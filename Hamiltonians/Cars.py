...
def LxF_Dubins_Plane(Phi, X, delta_x, delta_theta, rho, alpha, beta):
    ...
    gradU_xyz = (U_up_x - U_down_x)/(2*delta_x)    
    LapU_xyz = ((U_up_x + U_down_x - 2*U_center)/(2*delta_x)).sum(dim=-1)

    gradU_xy = gradU_xyz[:, :2]
    gradU_z = gradU_xyz[:, 2]
    ...
    x_term = -(CosSinTheta*gradU_xy).sum(-1)  #.abs()
    theta_term = gradU_theta.abs()/rho
    
    Lap_term = LapU_xyz + LapU_theta
    c = 1.
    z_term = gradU_z.abs() * c
    
    if alpha>0:
        alpha_sgn = alpha/abs(alpha)
        return x_term + theta_term - alpha*Lap_term + alpha_sgn*beta*U_center.squeeze() + z_term
    else:
        return x_term + theta_term - alpha*Lap_term + z_term

...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  4 12:08:59 2024
...
from Training.training import train_Gamma, train
...
def Central_line_ball(n):
    data = torch.zeros([n, dim])
    data[:, 0] = max(radii)*(2*torch.rand(n) - 1)
    return data
...
#Supervised data on Gamma
def h(X):
    return 2.*(6. - X.norm(dim = -1))
...
for i in range(rounds):
    ...
    t0 = t()
    total_loss = train_Gamma(NN, domain, training_params)
    t1 = t() - t0 
